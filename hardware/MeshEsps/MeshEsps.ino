...
String manageRequest(const String &request, ESP8266WiFiMesh &meshInstance) {
  ...
  objeto = digitalRead(2);
  tempo = millis();
   count = 0;
   while((millis() - tempo) < 1000){
     objeto = digitalRead(2);
     Serial.println(objeto);
      if(objeto == 0 && anterior == 1){
          count ++;
     }
     anterior = objeto;
   }  

    return ("chipid=" + meshInstance.getMeshName() + meshInstance.getNodeID() + "&sensor=" + count);
...
transmission_status_t manageResponse(const String &response, ESP8266WiFiMesh &meshInstance) {
  ...
  meshInstance.setMessage("chipid=" + meshInstance.getMeshName() + meshInstance.getNodeID() + "&sensor=" + count);
...
void loop() {
  ...
  if (millis() - timeOfLastScan > 6000 // Give other nodes some time to connect between data transfers.
    ...
    String request = "chipid=" +  meshNode.getMeshName() + meshNode.getNodeID()  + "&sensor=" + count;
